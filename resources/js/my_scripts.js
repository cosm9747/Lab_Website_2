...
function loadStatsPage(){
	...
	for(var i = 2; i < table.rows.length; i++){
		cellTwo = parseInt(table.rows[i].cells[2].innerHTML);
		cellThree = parseInt(table.rows[i].cells[3].innerHTML);
		if(cellTwo > cellThree){
...
function loadPlayersPage(){
	...
	for(var i = 0; i < players.length; i++){
		anchor = '<p><a href = # onclick= switchPlayers(' + i + ')>' + players[i].name + '</a></p>';
		tag += anchor;
	}
